 ...
 *
 *  This library is OPENSSL_free software; you can redistribute it and/or
 ...
 *
 * @param ctx context allocated by PKCS11_CTX_new()
 * @param slotnr the serialnumber for slot.if not is -1;
 * @param slots list of slots allocated by PKCS11_enumerate_slots()
 * @param nslots size of the list
 * @retval !=NULL pointer on a slot structure
 * @retval NULL error
 */
//add by zhoushenshen in 20150430
PKCS11_SLOT *PKCS11_find_token(PKCS11_CTX * ctx, unsigned long slotnr,
			PKCS11_SLOT *slots, unsigned int nslots);
...
extern PKCS11_KEY *PKCS11_find_pubkey(PKCS11_CERT *);
...
SM2_METHOD *PKCS11_get_SM2_method(void);

int PKCS11_get_ciphers(ENGINE *e, const EVP_CIPHER **cipher, const int **nids, int nid);

PKCS11_KEY *PKCS11_init_secret_key(PKCS11_TOKEN * token);

int PKCS11_sesskey_init_Operation(PKCS11_KEY *key,const unsigned char *sesskey,int nid,int enc);

int PKCS11_sesskey_Operation(PKCS11_KEY *key,const void *mech,unsigned char *out,
		      const unsigned char *in, unsigned int inl,int enc);
/**
...
#define CKM_SM4_KEY_GEN	(CKM_VENDOR_DEFINED+17)	
#define CKM_SM4_ECB		(CKM_VENDOR_DEFINED+18)
#define CKM_SM4_ECB_PAD	(CKM_VENDOR_DEFINED+19)
#define CKM_SM4_CBC		(CKM_VENDOR_DEFINED+20)
#define CKM_SM4_CBC_PAD	(CKM_VENDOR_DEFINED+21)
